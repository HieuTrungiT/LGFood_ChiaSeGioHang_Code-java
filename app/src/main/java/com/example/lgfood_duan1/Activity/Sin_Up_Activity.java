...
//        back về login
    ...
    private void checkValidateSet() {
        ...
        if (tenKH.length() < 6 || tenKH.length() > 100) {
            ...
            model_account = new model_Account(UUID.randomUUID().toString(), tenKH, userName, password, address, email, soDienThoai, UUID.randomUUID().toString(), UUID.randomUUID().toString(), UUID.randomUUID().toString(), "");
